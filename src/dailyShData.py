...
def continuousPullWrite(collection):
	...
	lat, lon, rad, units, name = localSearch_df.ix[row_index][['lat', 'lon', 'rad', 'units', 'name']]
	    ...
	    search_response = sh.searchEvents(lat, lon, rad, units)
            ...
            for event_object in search_response['events']: # Get inventory
		    ...
		    time1.sleep(6) # To avoid reaching api requests limit (10 requests/min). Currently at 1 api calls per loop
                    ...
                    continue
            try:
                history_dict[curr_day].append([lat, lon]) # Add entry to location search log
            except:
                history_dict[curr_day] = [[lat, lon]]
            print('SUCCESS | Pulled Event Information for eventId:{} on {}. API-Call Rate: {}. Mongo Collection Size: {}'.format(eventId, curr_day, (time() - st) / api_calls, collecton.count()))
        else:
